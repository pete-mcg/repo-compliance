...
```text
  ...
  | files, alerts, |  | rule. Static rules inspect|
...
- [`src/repo_compliance/runner.py`](../src/repo_compliance/runner.py) coordinates checks for each repository, skips exempt rules, confirms the `main` branch is accessible, and downloads one source archive when static analysis needs it.
