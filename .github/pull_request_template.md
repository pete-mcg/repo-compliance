<!--
The PR title must follow Conventional Commits.
Re-usable titles:
dev -> main: chore(release): merge dev into main for release.
-->

### Motivation

<!--
Why is this change required and what problem does it solve?
-->

<!--
If your PR changes more than 400 lines, uncomment this section below.
#### LARGE PR WARNING
Explain why the PR is large and why it cannot be reasonably split.
-->

### Description

<!--
Describe your changes and approach.
-->

### Contribution Checklist

<!--
Before submitting this PR, please make sure:
-->

- [ ] My PR is small, focused and fulfils a single purpose. Greater than 400 line changes are **by exception**.
- [ ] CI passes locally (`task ci`).
- [ ] I have ran the tool locally and interregoted the outputs, and not just relied on automated quality checks.
- [ ] I have updated relevant documentation.
- [ ] Where AI has been used, I have reviewed the code produced and can defend it.
- [ ] I have self-reviewed my code, following `CONTRIBUTING.md`.
