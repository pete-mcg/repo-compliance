...
def test_sends_authentication_version_and_timeout_headers() -> None:
    ...
    with GitHubClient(
        "test-token", transport=httpx.MockTransport(handle_request)
    ) as client:
        client.get_main_branch(REPOSITORY)
...
def test_reads_rule_and_classic_protection_responses() -> None:
    ...
    with GitHubClient("token", transport=httpx.MockTransport(handle_request)) as client:
        assert client.get_active_main_rule_types(REPOSITORY) == frozenset(
            {"deletion", "update"}
        )
        assert client.get_classic_deletion_setting(REPOSITORY) is False
...
def test_classic_protection_404_means_unprotected() -> None:
    ...
    with GitHubClient("token", transport=transport) as client:
        assert client.get_classic_deletion_setting(REPOSITORY) is None
...
def test_content_404_means_missing_file() -> None:
    ...
    with GitHubClient("token", transport=transport) as client:
        assert not client.has_file(REPOSITORY, ".github/CODEOWNERS")
...
def test_content_requires_exact_file_response_and_main_ref() -> None:
    ...
    with GitHubClient("token", transport=httpx.MockTransport(handle_request)) as client:
        assert client.has_file(REPOSITORY, ".github/CODEOWNERS")
        assert not client.has_file(REPOSITORY, ".github/CODEOWNERS")
...
def test_archive_download_follows_redirect_and_streams_file(tmp_path: Path) -> None:
    ...
    with GitHubClient("token", transport=httpx.MockTransport(handle_request)) as client:
        assert client.get_main_archive(REPOSITORY, destination) == destination
...
def test_timeout_is_wrapped_as_github_error() -> None:
    ...
    with (
        GitHubClient("token", transport=httpx.MockTransport(handle_request)) as client,
...
def test_invalid_api_responses_are_wrapped(method: str, payload: object) -> None:
    ...
    with (
        ...
        handle_client_method(client, method)
...
def test_archive_output_failure_is_wrapped(tmp_path: Path) -> None:
    ...
    with (
        ...
        client.get_main_archive(REPOSITORY, tmp_path / "missing" / "file.zip")


def handle_client_method(client: GitHubClient, method: str) -> None:
    if method == "branch":
        client.get_main_branch(REPOSITORY)
    elif method == "rules":
        client.get_active_main_rule_types(REPOSITORY)
    elif method == "protection":
        client.get_classic_deletion_setting(REPOSITORY)
    elif method == "content":
        client.has_file(REPOSITORY, ".github/CODEOWNERS")
    else:
